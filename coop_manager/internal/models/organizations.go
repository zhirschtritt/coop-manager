...
type organizationR struct {
	...
	ShiftSlots       ShiftSlotSlice       `boil:"ShiftSlots" json:"ShiftSlots" toml:"ShiftSlots" yaml:"ShiftSlots"`
...
func (r *organizationR) GetShiftSlots() ShiftSlotSlice {
	if r == nil {
		return nil
	}
	return r.ShiftSlots
...
func (q organizationQuery) OneG(ctx context.Context) (*Organization, error) {
...
func (q organizationQuery) AllG(ctx context.Context) (OrganizationSlice, error) {
...
// CountG returns the count of all Organization records in the query using the global executor
func (q organizationQuery) CountG(ctx context.Context) (int64, error) {
...
func (q organizationQuery) ExistsG(ctx context.Context) (bool, error) {
...
func (o *Organization) Members(mods ...qm.QueryMod) memberQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"member_organizations\" on \"members\".\"id\" = \"member_organizations\".\"member_id\""),
		qm.Where("\"member_organizations\".\"organization_id\"=?", o.ID),
	)
...
// ShiftSlots retrieves all the shift_slot's ShiftSlots with an executor.
func (o *Organization) ShiftSlots(mods ...qm.QueryMod) shiftSlotQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"shift_slots\".\"organization_id\"=?", o.ID),
	)
...
func (organizationL) LoadMembers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOrganization interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"members\".\"id\", \"members\".\"organization_id\", \"members\".\"created_at\", \"members\".\"updated_at\", \"members\".\"first_name\", \"members\".\"last_name\", \"members\".\"email\", \"members\".\"meta\", \"a\".\"organization_id\""),
		qm.From("\"members\""),
		qm.InnerJoin("\"member_organizations\" as \"a\" on \"members\".\"id\" = \"a\".\"member_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.OrganizationID, &one.CreatedAt, &one.UpdatedAt, &one.FirstName, &one.LastName, &one.Email, &one.Meta, &localJoinCol)
		if err != nil {
...
// LoadShiftSlots allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (organizationL) LoadShiftSlots(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOrganization interface{}, mods queries.Applicator) error {
	var slice []*Organization
	var object *Organization
	...
	query := NewQuery(
		qm.From(`shift_slots`),
		qm.WhereIn(`shift_slots.organization_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load shift_slots")
	}

	var resultSlice []*ShiftSlot
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice shift_slots")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on shift_slots")
	}
	if err = results.Err(); err != nil {
	...
	if singular {
		object.R.ShiftSlots = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.OrganizationID {
				local.R.ShiftSlots = append(local.R.ShiftSlots, foreign)
				if foreign.R == nil {
					foreign.R = &shiftSlotR{}
				}
				foreign.R.Organization = local
...
// AddCoopEventsG adds the given related objects to the existing relationships
...
func (o *Organization) AddCoopEventsG(ctx context.Context, insert bool, related ...*CoopEvent) error {
	return o.AddCoopEvents(ctx, boil.GetContextDB(), insert, related...)
}
...
// Uses the global database handle.
func (o *Organization) AddMembersG(ctx context.Context, insert bool, related ...*Member) error {
...
func (o *Organization) AddMembers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Member) error {
	...
	for _, rel := range related {
		query := "insert into \"member_organizations\" (\"organization_id\", \"member_id\") values ($1, $2)"
		values := []interface{}{o.ID, rel.ID}
...
// Sets o.R.Organizations's Members accordingly.
// Replaces o.R.Members with related.
// Sets related.R.Organizations's Members accordingly.
// Uses the global database handle.
func (o *Organization) SetMembersG(ctx context.Context, insert bool, related ...*Member) error {
...
// Replaces o.R.Members with related.
// Sets related.R.Organizations's Members accordingly.
func (o *Organization) SetMembers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Member) error {
	query := "delete from \"member_organizations\" where \"organization_id\" = $1"
	...
	removeMembersFromOrganizationsSlice(o, related)
	if o.R != nil {
		o.R.Members = nil
	}
...
// Uses the global database handle.
func (o *Organization) RemoveMembersG(ctx context.Context, related ...*Member) error {
...
// Removes related items from R.Members (uses pointer comparison, removal does not keep order)
// Sets related.R.Organizations.
func (o *Organization) RemoveMembers(ctx context.Context, exec boil.ContextExecutor, related ...*Member) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"member_organizations\" where \"organization_id\" = $1 and \"member_id\" in (%s)",
...
func removeMembersFromOrganizationsSlice(o *Organization, related []*Member) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
// AddMembershipTypesG adds the given related objects to the existing relationships
// of the organization, optionally inserting them as new records.
// Appends related to o.R.MembershipTypes.
// Sets related.R.Organization appropriately.
// Uses the global database handle.
func (o *Organization) AddMembershipTypesG(ctx context.Context, insert bool, related ...*MembershipType) error {
	return o.AddMembershipTypes(ctx, boil.GetContextDB(), insert, related...)
}
...
// Sets related.R.Organization appropriately.
// Uses the global database handle.
func (o *Organization) AddMembershipsG(ctx context.Context, insert bool, related ...*Membership) error {
...
// Sets related.R.Organization appropriately.
// Uses the global database handle.
func (o *Organization) AddShiftAssignmentsG(ctx context.Context, insert bool, related ...*ShiftAssignment) error {
...
// AddShiftSlotsG adds the given related objects to the existing relationships
// of the organization, optionally inserting them as new records.
// Appends related to o.R.ShiftSlots.
// Sets related.R.Organization appropriately.
// Uses the global database handle.
func (o *Organization) AddShiftSlotsG(ctx context.Context, insert bool, related ...*ShiftSlot) error {
	return o.AddShiftSlots(ctx, boil.GetContextDB(), insert, related...)
}

// AddShiftSlots adds the given related objects to the existing relationships
// of the organization, optionally inserting them as new records.
// Appends related to o.R.ShiftSlots.
// Sets related.R.Organization appropriately.
func (o *Organization) AddShiftSlots(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ShiftSlot) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.OrganizationID = o.ID
	...
	if o.R == nil {
		...
		o.R.ShiftSlots = append(o.R.ShiftSlots, related...)
	}
...
// AddShiftTermsG adds the given related objects to the existing relationships
// of the organization, optionally inserting them as new records.
// Appends related to o.R.ShiftTerms.
// Sets related.R.Organization appropriately.
// Uses the global database handle.
func (o *Organization) AddShiftTermsG(ctx context.Context, insert bool, related ...*ShiftTerm) error {
	return o.AddShiftTerms(ctx, boil.GetContextDB(), insert, related...)
}
...
// Sets related.R.Organization appropriately.
// Uses the global database handle.
func (o *Organization) AddShiftsG(ctx context.Context, insert bool, related ...*Shift) error {
...
func FindOrganizationG(ctx context.Context, iD string, selectCols ...string) (*Organization, error) {
	return FindOrganization(ctx, boil.GetContextDB(), iD, selectCols...)
}
...
func (o *Organization) InsertG(ctx context.Context, columns boil.Columns) error {
...
func (o *Organization) UpdateG(ctx context.Context, columns boil.Columns) (int64, error) {
...
func (q organizationQuery) UpdateAllG(ctx context.Context, cols M) (int64, error) {
...
// UpdateAllG updates all rows with the specified column values.
func (o OrganizationSlice) UpdateAllG(ctx context.Context, cols M) (int64, error) {
...
func (o *Organization) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
...
func (o *Organization) DeleteG(ctx context.Context) (int64, error) {
...
func (q organizationQuery) DeleteAllG(ctx context.Context) (int64, error) {
...
func (o OrganizationSlice) DeleteAllG(ctx context.Context) (int64, error) {
...
func (o *Organization) ReloadG(ctx context.Context) error {
	if o == nil {
...
func (o *OrganizationSlice) ReloadAllG(ctx context.Context) error {
	if o == nil {
...
func OrganizationExistsG(ctx context.Context, iD string) (bool, error) {
	return OrganizationExists(ctx, boil.GetContextDB(), iD)
}
