...
func testShiftSlotToOneOrganizationUsingOrganization(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadOrganization(ctx, tx, false, (*[]*ShiftSlot)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testShiftSlotToOneSetOpOrganizationUsingOrganization(t *testing.T) {
	var err error
...
var (
	shiftSlotDBTypes = map[string]string{`ID`: `uuid`, `OrganizationID`: `uuid`, `ShiftID`: `uuid`, `Name`: `text`, `Data`: `jsonb`}
	_                = bytes.MinRead
)
